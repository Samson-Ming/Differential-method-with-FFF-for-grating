...
RS_ref=[0.271904209737074   0.502698641670134];  %N=35,N_layer=20,N_int_pt=50;
...
Npt_l    = 50;           % lower number of layers
Npt_u    = 50;           % upper number of layers
...
for nMax =  nMax_l:nMax_step:nMax_u
   ...
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        [RS,s0V,TS,sSubV] = computeScatMatNVM_DM(lam,thI,epsB,Lam,epsS,xt,zV,epst,nMax,N,Npoints);
        else
        [RS,s0V,TS,sSubV,E]  =computeScatMatNVM_DM_Field(lam,thI,epsB,Lam,d,epsS,xt,xt_original,zV,epst,nMax,N,Npoints,StrucParam);
        end
        ...
        %errorR_P((nMax-nMax_l+nMax_step)/nMax_step,(N-Nly_l+Nly_step)/Nly_step) =1/numel(RPvec((nMax-nMax_l+nMax_step)/nMax_step,(N-Nly_l+Nly_step)/Nly_step,:))*sum(sum(sum((RPvec((nMax-nMax_l+nMax_step)/nMax_step,(N-Nly_l+Nly_step)/Nly_step,:)-RP_ref).^2./RP_ref.^2)));      
        RSvec((nMax-nMax_l+nMax_step)/nMax_step,(N-Nly_l+Nly_step)/Nly_step,:)  = abs((RS(index_R)').^2).*s0V(1,index_R)./s0V(1,nMax+1);
        errorR_S((nMax-nMax_l+nMax_step)/nMax_step,(N-Nly_l+Nly_step)/Nly_step) =1/numel(RSvec((nMax-nMax_l+nMax_step)/nMax_step,(N-Nly_l+Nly_step)/Nly_step,:))*sum(sum(sum((RSvec((nMax-nMax_l+nMax_step)/nMax_step,(N-Nly_l+Nly_step)/Nly_step,:)-RS_ref).^2./RS_ref.^2)));      
